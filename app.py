...
Satellite-based natural capital valuation for 64 districts
"""
...
# ── Custom CSS: black background, white text, Calibri/Arial font ────
st.markdown(
    """
    <style>
...
DISTRICT_AREAS = {
    ...
    "Tangail": 3414,
    "Bandarban": 4479, "Brahmanbaria": 1927, "Chandpur": 1704, "Chattogram": 5283,
    "Cumilla": 3085, "Cox's Bazar": 2492, "Feni": 928, "Khagrachhari": 2700,
    "Lakshmipur": 1456, "Noakhali": 3601, "Rangamati": 6116,
    "Bagerhat": 3959, "Chuadanga": 1174, "Jashore": 2606, "Jhenaidah": 1964,
    "Khulna": 4394, "Kushtia": 1601, "Magura": 1049, "Meherpur": 751,
    "Narail": 990, "Satkhira": 3858,
    "Bogura": 2920, "Chapai Nawabganj": 1703, "Joypurhat": 965, "Naogaon": 3436,
    "Natore": 1896, "Pabna": 2371, "Rajshahi": 2407, "Sirajganj": 2498,
    "Dinajpur": 3438, "Gaibandha": 2179, "Kurigram": 2245, "Lalmonirhat": 1247,
    "Nilphamari": 1547, "Panchagarh": 1405, "Rangpur": 2308, "Thakurgaon": 1781,
    "Jamalpur": 2032, "Mymensingh": 4363, "Netrokona": 2744, "Sherpur": 1364,
    "Habiganj": 2637, "Moulvibazar": 2599, "Sunamganj": 3670, "Sylhet": 3490,
    "Barguna": 1831, "Barishal": 2785, "Bhola": 3737, "Jhalokati": 749,
    "Patuakhali": 3221, "Pirojpur": 1308,
}
...
# ── Ecosystem coefficients (Taka / hectare / year) ─────────────────
ECO_COEFFICIENTS = {
    "wetland": 14785,
    "urban": 0,
    "cropland": 92,
    "vegetation": 232,
    "dense_vegetation": 969,
}
...
def classify_land_cover(ndvi, mean_ndvi, std_ndvi):
    """Return (label, coefficient) for a single NDVI value."""
...
# ── Exchange rate & carbon price tables ─────────────────────────────
...
def ndvi_anomaly(current, history):
    ...
    return {"mean": mu, "std": sigma, "anomaly": anom, "z": z,
            "label": label, "msg": msg}
...
def _ndvi_colormap():
    colors = [
        "#1a3a5c", "#5c4a2a", "#8b7355", "#c4a94d",
        "#8cc63f", "#4caf50", "#2e7d32", "#1b5e20",
    ]
...
def raster_to_rgba(ndvi_arr):
    """Convert an NDVI array to an RGBA uint8 image."""
    clipped = np.clip(ndvi_arr, -0.2, 0.9)
    norm = (clipped + 0.2) / 1.1
    rgba = NDVI_CMAP(norm)
    # Make nodata / NaN transparent
...
def read_raster(path):
    """Read single-band GeoTIFF, reproject to WGS84, return (data, (south,west,north,east))."""
...
def _legend_html():
    ...
    <div class="ndvi-legend">
        ...
        <div class="ndvi-legend-labels">
            ...
            <span>0.4</span><span>0.6</span><span>0.9</span>
        </div>
    </div>
    """
...
def make_simple_map(district, ndvi_val=None, status=None):
    m = folium.Map(
        location=[23.685, 90.356], zoom_start=7, tiles="OpenStreetMap"
    )
    coord = DISTRICT_COORDINATES.get(district)
    if coord:
        color = "red" if status and "Severe" in status else (
            "orange" if status and "Moderate" in status else "green"
        )
...
def _dark_layout(title, xtitle, ytitle, height=500):
    return dict(
        title=title, xaxis_title=xtitle, yaxis_title=ytitle,
        template="plotly_dark", height=height,
        paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)",
        font=dict(family="Calibri, Arial", color="white"),
        margin=dict(l=60, r=30, t=60, b=60),
    )


def ndvi_chart(df, preds, future_yrs, district):
    fig = go.Figure()
    fig.add_trace(go.Scatter(
        x=df["year"], y=df["ndvi"], mode="lines+markers",
        name="Historical NDVI", line=dict(color="#4CAF50", width=3),
        marker=dict(size=8, color="#4CAF50"),
    ))
    fig.add_trace(go.Scatter(
        x=future_yrs, y=preds, mode="lines+markers",
        name="Predicted NDVI", line=dict(color="#FF9800", width=3, dash="dash"),
        marker=dict(size=8, color="#FF9800"),
    ))
    fig.update_layout(_dark_layout(f"NDVI Trend -- {district} (1990-2040)", "Year", "NDVI"))
    return fig


def bar_chart(x, y, title, xlab, ylab, color="#4CAF50", height=400):
    fig = go.Figure(go.Bar(x=x, y=y, marker_color=color))
    fig.update_layout(_dark_layout(title, xlab, ylab, height))
    return fig
...
def pie_chart(labels, values, title):
    ...
    fig.update_layout(_dark_layout(title, "", "", 400))
    return fig
...
if "rasters" not in st.session_state:
    st.session_state.rasters = {}   # {year: {path, mean, std, bounds, rgba}}
...
def main():
    ...
    st.markdown(
        ...
        <div class="main-header">
            ...
            <p style="font-size:1.15rem; margin:6px 0 2px 0;">
                NDVI + NPP + Ecosystem Services + Future Predictions | 1990-2040
            </p>
            <p style="font-size:0.95rem; margin:0; color:#b0d0b0;">
                Satellite-based natural capital valuation for 64 districts
            </p>
        </div>
        """,
    ...
    with st.sidebar:
            ...
            if not HAS_RASTERIO:
                st.markdown(
                    ...
                    <div class="warning-box">
                        <b>rasterio is not installed.</b><br>
                        Install it with: <code>pip install rasterio</code><br>
                        Or switch to CSV Data Upload.
                    </div>
                    """,
                    unsafe_allow_html=True,
                )
                ...
                st.caption(
                    "Upload at least 5 GeoTIFF rasters (single-band NDVI) "
                    "for consecutive years or with a fixed interval."
                )
                raster_year = st.number_input("Year for this raster", 1990, 2024, 2020,
                                             key="ryear")
                raster_file = st.file_uploader("Select GeoTIFF (.tif / .tiff)",
                                              type=[".tif", ".tiff"], key="rfile")
                if st.button("Add Raster", key="add_raster_btn") and raster_file:
                    with st.spinner(f"Processing {raster_year} raster..."):
                        try:
                            ...
                            st.session_state.rasters[int(raster_year)] = {
                                "path": tmp_path, "mean": mu, "std": sig,
                                "bounds": bounds, "rgba": rgba_ds,
                            }
                ...
                if st.session_state.rasters:
                    ...
                    if len(yrs) >= 2:
                        ...
                        if len(unique_diffs) == 1:
                            ...
                            st.warning("Years are not evenly spaced -- predictions may be less accurate.")

        else:  # CSV
            csv_file = st.file_uploader("Upload CSV (columns: year, ndvi)", type=["csv"],
                                        key="csv_up")
            if csv_file:
                try:
                    ...
                    if "year" not in df_csv.columns or "ndvi" not in df_csv.columns:
                        ...
                        # Validate consecutive or fixed interval
                        diffs = list(df_csv["year"].diff().dropna())
                        unique = set(diffs)
                        if len(unique) == 1:
                            st.success(
                                f"Loaded {len(df_csv)} records -- "
                                f"Interval: every {int(diffs[0])} year(s)"
                            )
                        else:
                            st.warning(
                                f"Loaded {len(df_csv)} records -- "
                                f"Years are not evenly spaced."
                            )
        ...
        st.markdown("---")
        if (st.session_state.rasters and len(st.session_state.rasters) >= 5) or \
           (st.session_state.csv_data is not None and len(st.session_state.csv_data) >= 5):
            all_yrs = _get_all_years()
            if all_yrs:
                sel_year = st.slider("Select Year for Analysis",
                                     min(all_yrs), max(all_yrs), max(all_yrs))
    ...
    if not years_list or len(years_list) < 5:
        st.markdown(
            ...
            <div class="info-box">
                ...
                <ol>
                    <li>Select a division and district from the sidebar.</li>
                    <li>Upload at least 5 GeoTIFF rasters (one per year) or a CSV file
                        with 'year' and 'ndvi' columns.</li>
                    <li>Rasters must be single-band NDVI GeoTIFFs. Years should be
                        consecutive or follow a fixed interval.</li>
                    <li>Once loaded, use the year slider and explore the dashboard.</li>
                </ol>
                <p><b>CSV format example:</b></p>
                <pre style="background:#111;padding:8px;border-radius:4px;">
year,ndvi
2015,0.52
2016,0.55
2017,0.50
...
2019,0.58
    ...
    with c1:
        st.markdown(
            ...
            f'<h3>Current NDVI</h3><h2>{cur_ndvi:.3f}</h2>'
            f'<p>{anom["label"]}</p>'
            f'<small>Mean: {mean_ndvi:.3f} | Std: {std_ndvi:.3f}</small>'
            f'</div>', unsafe_allow_html=True)
    with c2:
        st.markdown(
            f'<div class="metric-card">'
            f'<h3>Total Economic Value</h3><h2>{total_val:.1f}</h2>'
            f'<p>Crore Taka / year</p>'
            f'<small>Carbon + Ecosystem Services</small>'
            f'</div>', unsafe_allow_html=True)
    with c3:
        st.markdown(
            f'<div class="metric-card">'
            f'<h3>Carbon Sequestration</h3><h2>{carb["carbon_tons"]/1e6:.2f}</h2>'
            f'<p>Million tons CO2e</p>'
            f'<small>${c_prices.get(sel_year,20):.1f} / ton</small>'
            f'</div>', unsafe_allow_html=True)
    with c4:
        st.markdown(
            f'<div class="metric-card">'
            f'<h3>Land Cover</h3><h2>{lc.replace("_"," ").title()}</h2>'
            f'<p>Eco Value: {eco_c:,} Tk/ha</p>'
            f'<small>{eco_val:.1f} Cr Tk/year</small>'
            f'</div>', unsafe_allow_html=True)

    if anom["z"] < -1:
        st.markdown(
            f'<div class="warning-box">'
            f'<h4>NDVI Anomaly Alert</h4>'
            f'<p>{anom["msg"]}</p>'
            f'<p>Anomaly: {anom["anomaly"]:.3f} (Z-score: {anom["z"]:.2f})</p>'
            f'</div>', unsafe_allow_html=True)
    ...
    if has_raster_data:
        view_yr = st.selectbox(
            "Select year to display on map",
            sorted(st.session_state.rasters.keys()),
            key="map_yr_select",
        )
        ...
        with col_stats:
            ...
            st.markdown(f"- South: {b[0]:.4f}")
            st.markdown(f"- West: {b[1]:.4f}")
            st.markdown(f"- North: {b[2]:.4f}")
            st.markdown(f"- East: {b[3]:.4f}")
    else:
    ...
    with col_ch1:
        fig_trend = ndvi_chart(df, preds, future_yrs, district)
        st.plotly_chart(fig_trend, use_container_width=True)
        st.caption(f"Prediction R-squared: {r2:.3f}")
    with col_ch2:
        # Carbon revenue historical
        revs = []
        ...
        fig_carb = bar_chart(
            df["year"].tolist(), revs,
            "Carbon Credit Revenue (Historical)", "Year", "Revenue (Cr Tk)",
        )
    ...
    with col_e1:
        fig_pie = pie_chart(
            ["Carbon Credits", "Land Cover Services", "Climate Regulation (est.)"],
            [carb["revenue_cr"], eco_val, eco_val * 0.3],
            "Ecosystem Service Value Distribution",
        )
        st.plotly_chart(fig_pie, use_container_width=True)
    with col_e2:
        fig_bar2 = bar_chart(
            ["Carbon Credits", "Land Cover", "Climate Reg."],
            [carb["revenue_cr"], eco_val, eco_val * 0.3],
            "Service Values (Cr Tk/year)", "Service", "Value (Cr Tk)",
            color="#FF9800",
        )
    ...
    if len(df) >= 2:
        ...
        if len(prev_row):
            ...
            si = stress_index(cur_ndvi, prev_n,
                              ex_rates.get(sel_year, 100), ex_rates.get(prev_y, 100))
            st.markdown("## Agricultural Stress Index")
            box_cls = "warning-box" if si["level"] != "Normal" else "success-box"
            st.markdown(
                f'<div class="{box_cls}">'
                f'<h4>Stress Level: {si["level"]}</h4>'
                f'<p>{si["msg"]}</p>'
                f'<p>Exchange rate change: {si["ex_chg"]:+.2f}% | '
                f'NDVI change: {si["nd_chg"]:+.2f}% | '
                f'Index: {si["si"]:.2f}</p>'
                f'</div>', unsafe_allow_html=True)
    ...
    for i, (y, p) in enumerate(zip(future_yrs, preds)):
        ...
        rows.append({
            "Year": y,
            "Predicted NDVI": f"{p:.3f}",
            "Change (%)": f"{chg:+.2f}" if chg else "--",
            "Carbon Revenue (Cr Tk)": f"{c_pp['revenue_cr']:.1f}",
        })
    ...
    with d1:
        st.download_button("Historical NDVI (CSV)", df.to_csv(index=False),
                           f"{district}_historical_ndvi.csv", "text/csv")
    with d2:
        st.download_button("Future Predictions (CSV)",
                           pd.DataFrame(rows).to_csv(index=False),
                           f"{district}_future_predictions.csv", "text/csv")
    with d3:
        full = df.copy()
        full["land_cover"] = [classify_land_cover(n, mean_ndvi, std_ndvi)[0]
                              for n in full["ndvi"]]
        st.download_button("Complete Analysis (CSV)", full.to_csv(index=False),
                           f"{district}_complete_analysis.csv", "text/csv")

    # ── Footer ──────────────────────────────────────────────────────
    st.markdown("---")
    st.markdown(
        '<div style="text-align:center;color:#666;">'
        '<p>ECO-VISION: BANGLADESH </p>'
        '<p><i>Protect vegetation = Protect income</i></p>'
...
def _get_series():
    """Return (years_list, ndvi_list) from whichever source is active."""
    if st.session_state.rasters:
        yrs = sorted(st.session_state.rasters.keys())
